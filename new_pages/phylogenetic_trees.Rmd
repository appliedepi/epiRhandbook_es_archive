...
ggtree(tree, layout="circular", branch.length='none') %<+% sample_data + # %<+% añade el dataframe con datos de muestra al árbol
  ...
  scale_color_manual(name = "Sample Origin", # nombre de tu esquema de color (aparecerá en la leyenda así) 
                   ...
                   labels = c("NRCSS Belgium", "Other"), # cómo quieres que se nombren las diferentes opciones en tu leyenda, sirve para formatearlas
  ...
  new_scale_color()+ # permite añadir un esquema de color adicional para otra variable
     geom_tippoint(aes(color=Continent), size=1.5)+ # color de la punta por continent. Puedes cambiar la forma añadiendo "shape = "
...
pacman::p_load(
  ...
  ggnewscale)      # para añadir capas adicionalales de esquemas de color
...
ggtree(tree, layout = "circular", branch.length = 'none') %<+% sample_data + # %<+% adds dataframe with sample data to tree
  ...
  new_scale_color()+                             # permite añadir un esquema de color adicional para otra variable
    geom_tippoint(
      mapping = aes(color = Continent),          # color de la punta por continent. Puedes cambiar la forma añadiendo "shape = "
      size = 1.5)+                               # define el tamaño del punto en la punta
  scale_color_brewer(
  ...
  theme(
    ...
    legend.title = element_text(    # define el tamaño y el formato de la fuente del título de la leyenda
      face = "bold",
      size = 12),   
    legend.text=element_text(       # define el tamaño y el formato de la fuente del texto de la leyenda
      face = "bold",
      size = 10),  
    plot.title = element_text(      # define el tamaño y el formato de la fuente del título del gráfico
      size = 12,
      face = "bold"),  
...
p <- ggtree(tree,) %<+% sample_data +
  geom_tiplab(size = 1.5) +                # etiqueta las puntas de todas las ramas con el nombre de la muestra en el árbol
  geom_text2(
...
ggtree(
  ...
  geom_tiplab(size = 1)+                                # etiqueta las puntas de todas las ramas con el nombre de la muestra en el árbol
  geom_text2(
    mapping = aes(subset = !isTip, label = node),
    size = 3,
    color = "darkred") +                                # etiqueta todos los nodos del árbol
 theme(
   legend.position = "none",                            # elimina la leyenda en su totalidad
...
p1 <- p + geom_hilight(  # resalta el nodo 39 en azul, "extend =" permite definir la longitud del color block
  node = 39,
  fill = "steelblue",
  extend = 0.0017) +  
...
ggtree(sub_tree2) %<+% sample_data +     # usamos el operador %<+% para enlazar con sample_data
  geom_tiplab(                          # etiqueta las puntas de todas las ramas con el nombre de la muestra en el árbol
    size = 2.5,
    offset = 0.001,
    align = TRUE) + 
  theme_tree2()+
  xlim(0, 0.015)+                       # establece los límites del eje x de nuestro árbol
  geom_tippoint(aes(color=Country),     # colorea el punto de la punta por continent
                size = 1.5)+ 
  scale_color_brewer(
    name = "Country", 
    palette = "Set1", 
  ...
  geom_tiplab(                          # añade el historial de viajes como una etiqueta de texto en las puntas, en color rojo
    ...
    geom = "text",
    align = TRUE)+ 
  ggtitle("Phylogenetic tree of Belgian S. sonnei strains with travel history")+  # añade el título de la gráfica
...
h1 <-  gheatmap(p, gender,                            # añadimos una capa de mapa de calor del dataframe de género a nuestro gráfico de árbol
...
h3 <- gheatmap(h2, cipR,         # añade la segunda fila del mapa de calor que describe las mutaciones de resistencia a la ciprofloxacina
               offset = 12, 
               width = 0.10, 
               colnames = FALSE) +
...
# Primero añadimos el nuevo esquema de coloración:
