...
Esta página demuestra el uso del paquete **stringr** para evaluar y manejar valores de caracteres, también denominados "cadenas".
...
Gran parte de la inspiración para esta página se obtuvo de esta [viñeta de stringr](https://cran.r-project.org/web/packages/stringr/vignettes/stringr.html).
...
pacman::p_load(
  stringr,    # funciones para trabajar con cadenas
  tidyverse,  # manipulación de datos 
  tools)      # alternativa para convertir cadenas a mayúsculas
...
Utiliza `str_glue()` para insertar código R dinámico en una cadena. Se trata de una función muy útil para crear títulos o pies de gráfico dinámicos, como se demuestra a continuación. 
...
Dentro de un dataframe, la unión de valores de caractéres de varias columnas puede lograrse con `unite()` de **tidyr**. Esto es lo contrario de `separate()`. 

Indique el nombre de la nueva columna (que contendrá los valores unidos) y, a continuación, indique los nombres de las columnas que desea unir. 
...
También puede ajustar el número de divisiones a crear con el argumento `n =`. Por ejemplo, esto restringe el número de divisiones a 2. De este modo, cualquier otra coma permanece dentro del segundo valor. 
...
Para buscar con varios términos, inclúyelos separados por barras O (`|`) dentro del argumento `pattern =`, como se muestra a continuación: 
