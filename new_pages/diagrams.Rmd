...
* Diagramas aluviales/sanitarios
* Calendario de eventos9
...
La mayor parte del contenido de esta página no requiere unos datos. Sin embargo, en la sección del diagrama de Sankey, utilizaremos la lista de casos de una epidemia de ébola simulada. Si quiere seguir esta parte, [clica para descargar linelist "limpio"](https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_cleaned.rds) (como archivo .rds). Importa los datos con la función `import()` del paquete **rio** (maneja muchos tipos de archivos como .xlsx, .csv, .rds - vea la página [importación y exportación](#import-and-export) para más detalles).
...
3)  Define  el gráfico (layout, rank direction)
4)  Declara los nodos (create nodes)
5)  Declara las aristas (da enlaces entre nodos)
...
Los nombres de los nodos, o las declaraciones de las aristas (edges), pueden separarse con espacios, punto y coma o nuevas líneas.
...
Los nombres de los nodos pueden ser palabras sueltas, como en el sencillo ejemplo anterior. Para utilizar nombres de varias palabras o caracteres especiales (por ejemplo, paréntesis, guiones), ponga el nombre del nodo entre comillas simples (' '). Puede ser más fácil tener un nombre de nodo corto, y asignar una *etiqueta* como se muestra a continuación entre corchetes `[ ]`. Si deseas tener una nueva línea dentro del nombre del nodo, debes hacerlo a través de una etiqueta - utiliza `\n` en la etiqueta del nodo entre comillas simples, como se muestra a continuación.

**Subgrupos** 

En las declaraciones de aristas, se pueden crear subgrupos a ambos lados de la arista con corchetes (`{ }`). La arista se aplica entonces a todos los nodos en el corchete - es una forma abreviada.
...
El siguiente ejemplo amplía el surveillance_diagram, añadiendo nombres de nodos complejos, aristas agrupadas, colores y estilos
...
Te animamos a revisar este tutorial si la parametrización es algo que le interesa.
...
<!-- ======================================================= -->
## Alluvial/Sankey Diagrams {#alluvialsankey-diagrams}
...
Ahora creamos un dataframe de todos los nodos del diagrama, bajo la columna. `name`. Esto consiste en todos los valores de `hospital` y `age_cat.` Observa que nos aseguramos de que todos son caracteres de tipo antes de combinarlos. y ajustamos las columnas ID para que sean números en lugar de etiquetas:
...
Este es un ejemplo en el que también se incluye el resultado del paciente. Obsérvese que en el paso de preparación de los datos tenemos que calcular los recuentos de casos entre la edad y el hospital, y por separado entre el hospital y el resultado - y luego unir todos estos recuentos con `bind_rows()`.
